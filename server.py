...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        '''
        The server_socket.bind((host, port)) line connects the server_socket we have to an IP and port, and then the
        listen() function tells the socket to wait and "listen" to any attempts to connect from a client
        '''
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                '''
                The function server_socket.accept() returns a tuple, containing the "client_socket" - the connection 
                from the client side, and the "address" , the IP and port number of the client and connection. The
                server_socket stops listening as it is now connected to a client
                '''
                # *Fill in end(2)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            to the assignment is at most 8180, so we pass 8192 (api.BUFFER_SIZE/8) as the buffer argument
            '''
            # * Fill in end (3)
            if not data:
                break
            try:
                ...
                '''
                The sendall(response) function sends to the client all the data in the response variable, it handles
                buffering on it's own and makes sure all the data in response is sent
                '''
                # * Fill in end (4)
