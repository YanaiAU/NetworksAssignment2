...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        proxy_socket.listen()
        '''
        The proxy_socket.bind(proxy_address) line connects the proxy_socket we have to an IP and port, and then the
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                client_socket, client_address = (proxy_socket, client_address) = proxy_socket.accept()
                '''
                The function proxy_socket.accept() returns a tuple, containing the "client_socket" - the connection 
